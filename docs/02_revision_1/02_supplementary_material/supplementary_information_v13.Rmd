...
# Study Protocol, Data Quality, and Analytic Sample

```{r descriptives-setup}
# Define sample characteristics
N_subj <- 119
ema_scheduled <- 27
ema_completed_mean <- 21.00
ema_completed_sd <- 4.16
ema_completed_median <- 22
ema_completed_min <- 5
ema_completed_max <- 26
ema_compliance_mean <- 77.8
ema_compliance_median <- 81.5
ema_compliance_min <- 18.5
ema_compliance_max <- 96.3
voice_obs_total <- 357
voice_obs_per_period <- 119
```

Descriptive and quality-control information is reported for $N$ = `r N_subj` female participants. Each participant could receive up to `r ema_scheduled` EMA notifications across the broader course-assessment period. The selected participants completed an average of `r ema_completed_mean` EMA assessments ($SD$ = `r ema_completed_sd`, median = `r ema_completed_median`, range = `r ema_completed_min`--`r ema_completed_max`), corresponding to a mean compliance rate of `r ema_compliance_mean` percent.

## Design and EMA/Voice Coverage

Table \@ref(tab:table-s3-design) summarizes the planned EMA schedule, completed EMA assessments, compliance, and voice data coverage. All `r N_subj` participants contributed usable voice observations in each assessment period (Baseline, Pre-exam, Post-exam), yielding `r voice_obs_total` total voice observations.

```{r table-s3-design}
# Crea la tabella con due righe
S3_design <- data.frame(
  Metric = c(
    "Sample size",
    "EMA schedule",
    "EMA completion",
...
knitr::kable(
  S3_design,
  col.names = c("Metric", "Statistic", "Value"),
  caption = "Study design, EMA compliance, and voice-data coverage.",
  escape = FALSE,
  booktabs = TRUE
)
```

## EMA Protocol and Compliance {#s-ema-protocol}
...
## EMA Response-Quality Screening {#s-ema-quality}
...
Participants were classified as careless or insufficient-effort responders if their composite response-quality score exceeded 2.5 standard deviations above the sample mean. This criterion was intentionally conservative: participants were not excluded solely because they showed an isolated anomaly on one response-quality indicator, but only when their overall response profile was clearly atypical across the combined screening metrics.

This participant-level screening procedure resulted in the exclusion of 8 participants from the initial eligible sample of 131 participants, corresponding to 6.11% of the sample. This exclusion rate reflects a conservative data-cleaning approach intended to balance two goals: removing response protocols with the strongest evidence of insufficient-effort responding while minimizing the risk of false-positive exclusions and preserving statistical power.


## Voice-Recording Quality Control and Final Analytic Samples {#s-voice-qc}

Voice recordings were collected at baseline, pre-exam, and post-exam around the first intermediate test. The primary analyses required complete sustained-vowel recordings across all three sessions plus valid EMA data. Audio files were screened independently from EMA data for integrity, task compliance, steady-state phonation length, and artifacts. Of the 131 initial participants, 8 were excluded for EMA quality control and 4 for recording screening, leaving 119 participants and yielding 357 sustained-vowel observations.

\newpage

# Personality Measures and Reliability
...
# Acoustic Measures, Manipulation Check, and Descriptives
...
threshold. Second, *direction is better identified than magnitude.* As shown in the prior-sensitivity analysis of Section \@ref(s-prior-sensitivity), the sign and
...
Models were estimated using Hamiltonian Monte Carlo with the No-U-Turn Sampler. Unless otherwise noted, models were run with 4 chains and at least 4,000 iterations per chain, including 2,000 warmup iterations. Convergence and sampling quality were evaluated using $\hat{R}$ < 1.01, effective sample size > 400, the absence of divergent transitions, and inspection of sampler diagnostic warnings.

Sampler control parameters were set to a conservative fitting configuration (adapt_delta = 0.95, max_treedepth = 18). These settings were chosen a priori to reduce the risk of sampling pathologies, but were not treated as substitutes for diagnostic evaluation. If divergent transitions had been observed, adapt_delta would have been increased; if chains had saturated the tree-depth limit, max_treedepth would have been increased. No additional tightening of these control parameters was required for the reported lower-dimensional models.

The higher-dimensional personality-moderation models were estimated with a longer configuration: 4 chains of 8,000 iterations each, including 2,000 warmup iterations, yielding 24,000 post-warmup draws. For these models, we used adapt_delta = 0.99 and max_treedepth = 15. When preliminary fits indicated convergence difficulties or unstable variance-component estimates, we increased the number of iterations and, where necessary, simplified random-effects structures before fitting the final reported models. All reported models satisfied the convergence and sampling-quality criteria described above.
...
## Posterior Predictive Directional Assurance of Moderation Effects {#s-assurance}
...
The MFCC analysis was restricted to participants with usable connected-speech recordings at all three sessions. Complete and quality-checked connected-speech recordings were available for 127 participants, which contributed to the MFCC models (127 participants $\times$ 3 sessions = 381 observations). Because this robustness check uses only the connected-speech signal, it does not require the sustained-vowel or EMA data; the 127-participant connected-speech subsample therefore contains the smaller main analytic sample ($N$ = 119), defined as participants with usable recordings for *both* tasks and valid EMA.

### Model Specification

The 13 coefficient means were standardized ($z$-scored across all observations) so that effects are expressed in SD units and the priors operate on a common scale. The outcome vector was modeled with a multivariate normal likelihood,
...
## Within-Person Temporal Covariation {#s-within-person-covariation}
...
# Model Diagnostics {#s-model-diagnostics}
