...
\section{Study Protocol, Data Quality, and Analytic Sample}\label{study-protocol-data-quality-and-analytic-sample}

Descriptive and quality-control information is reported for \(N\) = 119 female participants. Each participant could receive up to 27 EMA notifications across the broader course-assessment period. The selected participants completed an average of 21 EMA assessments (\(SD\) = 4.16, median = 22, range = 5--26), corresponding to a mean compliance rate of 77.80 percent.

\subsection{Design and EMA/Voice Coverage}\label{design-and-emavoice-coverage}

Table \ref{tab:table-s3-design} summarizes the planned EMA schedule, completed EMA assessments, compliance, and voice data coverage. All 119 participants contributed usable voice observations in each assessment period (Baseline, Pre-exam, Post-exam), yielding 357 total voice observations.

\begin{table}

\caption{\label{tab:table-s3-design}Study design, EMA compliance, and voice-data coverage.}
\centering
\begin{tabular}[t]{lll}
\toprule
...
\midrule
Sample size & N & 119\\
EMA schedule & Scheduled notifications & 27\\
EMA completion & M & 21.00\\
EMA completion & SD & 4.16\\
EMA completion & Median & 22\\
\addlinespace
EMA completion & Range & 5--26\\
Compliance & M & 77.8\%\\
Compliance & Median & 81.5\%\\
Compliance & Range & 18.5--96.3\%\\
Voice data & Total observations & 357\\
\addlinespace
Voice data & Per period & 119\\
\bottomrule
\end{tabular}
\end{table}

\subsection{EMA Protocol and Compliance}\label{s-ema-protocol}
...
\subsection{EMA Response-Quality Screening}\label{s-ema-quality}
...
Careless responding was operationalized using three complementary indices computed with the R package \texttt{\{careless\}}. First, we computed the \textbf{Longstring Index}, defined as the maximum number of consecutive identical responses provided by a participant. This index was used to capture invariant responding or straightlining. Second, we computed \textbf{Individual Response Variability} to quantify within-person variability in EMA responses; unusually low variability was interpreted as a potential indicator of non-differentiated responding. Third, we computed \textbf{Mahalanobis Distance} to identify participants whose multivariate response profiles were atypical relative to the rest of the sample.
...
\subsection{Voice-Recording Quality Control and Final Analytic Samples}\label{s-voice-qc}
...
\section{Personality Measures and Reliability}\label{personality-measures-and-reliability}

\subsection{Reliability Overview}\label{reliability-overview}
...
The brief PID-5 administered via EMA comprised 15 items (3 per domain), selected from the full 220-item instrument via a structured psychometric procedure (see \emph{Item Selection}, below). Because these items were assessed repeatedly across approximately 20 occasions per participant, standard single-level reliability estimates are inappropriate. Instead, we employed the multilevel composite reliability framework described by Lai (2021), which partitions variance into within-person and between-person components and computes separate reliability indices for each level.

\subsubsection{Item Selection}\label{item-selection}

The 15 EMA items were drawn from the full 220-item PID-5 using a structured item-selection procedure applied to an independent Italian PID-5 sample (\emph{N} = 1,183; \emph{M}\textsubscript{age} = 31.3, \emph{SD} = 14.8; 46.2\% female; Sica et al., 2024). The aim was to retain, for each PID-5 facet, the single item that best represented its facet and domain while remaining suitable for brief, repeated momentary administration. Selection combined two complementary criteria:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Factor loadings.} For each domain, an exploratory factor analysis (maximum-likelihood extraction) identified items with strong loadings (\(\geq\) .40) on the underlying domain factor.
\item
  \textbf{Scalability.} A Mokken scale analysis evaluated item scalability via Loevinger's \(H\) coefficients, retaining items with \(H \geq\) .30 as an index of discrimination.
\end{enumerate}

The final items were those that performed consistently well under both criteria. Because each of the five PID-5 domains is defined by three facets, this procedure yielded one item per facet---three items per domain, 15 in total---so that the brief scale preserves the full facet-level content structure of each domain rather than sampling a domain narrowly (Table \ref{tab:ema-items}). For EMA administration, the selected items were reworded from their dispositional (``trait'') phrasing to a current-state phrasing (e.g., \emph{``I worry a lot about being alone''} \(\rightarrow\) \emph{``At this moment, I worry a lot about being alone''}); item content was otherwise unchanged.
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2500}}@{}}
\caption{\label{tab:ema-items} EMA-based brief PID-5 item selection.}\tabularnewline
\toprule\noalign{}
...
Item
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
PID-5 item (official English wording)
\end{minipage} \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
...
\section{Acoustic Measures, Manipulation Check, and Descriptives}\label{acoustic-measures-manipulation-check-and-descriptives}
...
\begin{tabular}[t]{l>{\raggedleft\arraybackslash}p{2.8cm}>{\raggedleft\arraybackslash}p{2.8cm}c}
\toprule
Contrast & Posterior median & 89\% CrI & Directional probability\\
\midrule
\addlinespace[0.3em]
\multicolumn{4}{l}{\textbf{Negative affect composite (0--400)}}\\
\hspace{1em}PRE - BASELINE & 43.92 & {}[31.73, 55.99] & Pr($\Delta$ > 0) = 1.00\\
\hspace{1em}POST - PRE & -69.70 & {}[-81.81, -57.59] & Pr($\Delta$ < 0) = 1.00\\
\addlinespace[0.3em]
\multicolumn{4}{l}{\textbf{Perceived threat (0--5)}}\\
\hspace{1em}PRE - BASELINE & 0.60 & {}[0.43, 0.77] & Pr($\Delta$ > 0) = 1.00\\
\addlinespace[0.3em]
\multicolumn{4}{l}{\textbf{Pleasantness (-2--2)}}\\
\hspace{1em}PRE - BASELINE & -0.61 & {}[-0.76, -0.46] & Pr($\Delta$ < 0) = 1.00\\
\addlinespace[0.3em]
\multicolumn{4}{l}{\textbf{Negative appraisal composite (0--10)}}\\
\hspace{1em}PRE - BASELINE & 1.36 & {}[1.03, 1.70] & Pr($\Delta$ > 0) = 1.00\\
\hspace{1em}POST - PRE & -1.88 & {}[-2.22, -1.54] & Pr($\Delta$ < 0) = 1.00\\
\hspace{1em}POST - BASELINE & -0.52 & {}[-0.86, -0.18] & Pr($\Delta$ < 0) = 0.99\\
\bottomrule
\end{tabular}}
\end{table}

\subsection{Acoustic Features: Analyzed Parameters and Full Extracted Set}\label{s-acoustic-features}
...
\section{Statistical Models and Sensitivity Analyses}\label{s-statistical-models}
...
Two consequences of this framework are worth making explicit. First, \emph{we draw conclusions
...
threshold. Second, \emph{direction is better identified than magnitude.} As shown in the prior-sensitivity analysis of \ref{s-prior-sensitivity}, the sign and
...
Models were estimated using Hamiltonian Monte Carlo with the No-U-Turn Sampler. Unless otherwise noted, models were run with 4 chains and at least 4,000 iterations per chain, including 2,000 warmup iterations. Convergence and sampling quality were evaluated using \(\hat{R}\) \textless{} 1.01, effective sample size \textgreater{} 400, the absence of divergent transitions, and inspection of sampler diagnostic warnings.

Sampler control parameters were set to a conservative fitting configuration (adapt\_delta = 0.95, max\_treedepth = 18). These settings were chosen a priori to reduce the risk of sampling pathologies, but were not treated as substitutes for diagnostic evaluation. If divergent transitions had been observed, adapt\_delta would have been increased; if chains had saturated the tree-depth limit, max\_treedepth would have been increased. No additional tightening of these control parameters was required for the reported lower-dimensional models.

The higher-dimensional personality-moderation models were estimated with a longer configuration: 4 chains of 8,000 iterations each, including 2,000 warmup iterations, yielding 24,000 post-warmup draws. For these models, we used adapt\_delta = 0.99 and max\_treedepth = 15. When preliminary fits indicated convergence difficulties or unstable variance-component estimates, we increased the number of iterations and, where necessary, simplified random-effects structures before fitting the final reported models. All reported models satisfied the convergence and sampling-quality criteria described above.
...
\begin{table}[!h]
\centering
\caption{\label{tab:table-f0-main-effects-robust}Comparison of Gaussian and robust Student-t main-effects models for F0.}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{lrrrc}
\toprule
Parameter & Median & MAD & 89\% CrI & Directional probability\\
\midrule
\addlinespace[0.3em]
\multicolumn{5}{l}{\textbf{Gaussian}}\\
\hspace{1em}Stress, $b_1$ & 3.27 & 1.25 & {}[1.25, 5.27] & P($b_1$ > 0) = 0.995\\
\hspace{1em}Recovery, $b_2$ & 0.14 & 1.24 & {}[-1.89, 2.11] & P($b_2$ > 0) = 0.542\\
\addlinespace[0.3em]
\multicolumn{5}{l}{\textbf{Robust Student-t}}\\
\hspace{1em}Stress, $b_1$ & 2.30 & 1.14 & {}[0.47, 4.16] & P($b_1$ > 0) = 0.979\\
\hspace{1em}Recovery, $b_2$ & -0.39 & 1.14 & {}[-2.21, 1.41] & P($b_2$ > 0) = 0.368\\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[!h]
\centering
\caption{\label{tab:table-nne-main-effects-robust}Comparison of Gaussian and robust Student-t main-effects models for NNE.}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
...
\begin{table}[!h]
\centering
\caption{\label{tab:table-psis-loo-robust-comparison}PSIS-LOO predictive comparison of Gaussian and robust Student-t main-effects models.}
\centering
\resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{lllll}
\toprule
...
\subsection{Prior-Sensitivity Analysis}\label{s-prior-sensitivity}
...
\subsection{Posterior Predictive Directional Assurance of Moderation Effects}\label{s-assurance}
...
\section{Additional and Robustness Analyses}\label{s-additional-robustness}

\subsection{Connected-Speech Spectral Features: Mel-Frequency Cepstral Coefficient (MFCC) Analysis}\label{s-mfcc}

\subsubsection{Rationale and Feature Extraction}\label{rationale-and-feature-extraction}
...
\subsubsection{Omnibus Tests}\label{omnibus-tests}
...
\subsubsection{Per-Coefficient Effects}\label{per-coefficient-effects}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1429}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1429}}@{}}
\caption{\label{tab:mfcc-effects} Per-coefficient stress and recovery effects for mel-frequency cepstral coefficients.}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Coefficient
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\subsubsection{Multivariate Effect Size}\label{multivariate-effect-size}
...
\subsubsection{No Personality Moderation on Connected Speech}\label{no-personality-moderation-on-connected-speech}
...
\subsection{Voice--Affect Coupling}\label{s-voice-affect-coupling}
...
\subsection{Within-Person Temporal Covariation}\label{s-within-person-covariation}
...
{\centering \includegraphics[width=1\linewidth]{../../../results/F0/diagnostics/trace_main_effects} 

}

\caption{Trace plots for main fixed effects parameters showing good mixing across four chains.}\label{fig:trace-figure}
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{../../../results/F0/diagnostics/ppc_density_overlay} 

}

\caption{Posterior predictive check showing 100 draws from the posterior predictive distribution overlaid on the observed F0 distribution.}\label{fig:ppc-figure}
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{../../../results/NNE/diagnostics/ppc_density_overlay} 

}

\caption{Posterior predictive check for the NNE moderation model, showing posterior predictive density overlaid on the observed NNE distribution.}\label{fig:ppc-nne-figure}
\end{figure}
