...
The 15 EMA items were selected from the full 220-item PID-5 through a structured item-selection procedure applied to an independent Italian PID-5 sample [*N* = 1,183&#59; $M_{\mathrm{age}}$ = 31.3, *SD* = 14.8&#59; 46.2% female&#59; see @bottesi2025advancing; @sica2024comparing]. The aim was to retain, for each PID-5 facet, the single item that best represented its facet and domain while remaining suitable for brief, repeated momentary administration. Selection combined two complementary criteria:
...
[PLACEHOLDER: Sample for this analysis — [N_MFCC] participants contributed usable connected-speech recordings at all three sessions ([N_OBS] = [N_MFCC] $\times$ 3 observations). Reconcile this count with the main analytic sample ($N$ = 119) and with the recruited sample, and state the reason for any difference (e.g., connected-speech recordings excluded for file quality or background noise).]
