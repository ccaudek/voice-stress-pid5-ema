...
This study revealed three key findings about personality, stress, and voice: 
(1) academic stress produced small but consistent changes in acoustic parameters, 
primarily pitch elevation and improved voice quality; (2) personality pathology 
dimensions, measured ecologically via EMA, moderated these stress 
responses (9 credible interactions); (3) a brief EMA assessment (15 items) matched 
comprehensive questionnaires (220 items) in predictive validity, while offering 
ecological and temporal advantages.

### Main Effects: How Does Stress Alter Voice?

Academic stress increased fundamental frequency by 3-4 Hz across vowels (d = 0.15-0.16), 
consistent with physiological arousal theories linking sympathetic activation to 
increased vocal fold tension (Scherer et al., 2013). However, stress also **improved** 
certain voice quality metrics—reducing jitter (d = -0.17) and noise (NNE, d = -0.25).

This counterintuitive pattern likely reflects **performance arousal**: under evaluative 
pressure, speakers may increase articulatory precision and vocal control, similar to 
athletes showing enhanced motor performance during competition (Laukka et al., 2008). 
The concurrent reduction in pitch variability (more monotone speech) suggests cognitive 
load constraining prosodic expressiveness (Bänziger & Scherer, 2005)—a "tunnel vision" 
effect where attention to task performance reduces spontaneous vocal modulation.

The **absence of post-exam recovery** indicates that stress persists beyond stressor 
completion, consistent with anticipatory anxiety about outcomes. This temporal pattern 
underscores the need for longitudinal designs capturing stress dynamics, not just 
peak moments.

Critically, these main effects were **small** (|d| < 0.25), establishing that average 
stress responses show modest acoustic signatures. This motivated our focus on individual 
differences: **personality moderation effects** matched or exceeded main effect sizes, 
demonstrating substantial person-specific variability in stress reactivity.

### Personality Moderation: Individual Differences in Stress Reactivity

Against the backdrop of small main effects, personality pathology dimensions 
**significantly moderated** vocal stress responses (9 credible interactions). 
Individuals high in Negative Affectivity showed amplified pitch increases under 
pre-exam stress (β = 5.81 Hz, nearly **doubling** the average effect for those 
one SD above the mean). This suggests that trait negative emotionality confers 
stress **vulnerability**: heightened arousal translating directly into acoustic 
changes visible to conversational partners.

Antagonism and Detachment showed distinct patterns, primarily moderating **recovery 
dynamics** rather than acute stress responses. Antagonistic individuals exhibited 
slower pitch normalization post-exam, potentially reflecting sustained defensive 
arousal or difficulties disengaging from evaluative contexts. Detachment predicted 
reduced formant variability recovery, possibly indicating social-cognitive withdrawal 
manifesting in reduced articulatory expressiveness.

These domain-specific patterns align with DSM-5 Alternative Model conceptualizations: 
Negative Affectivity captures emotional reactivity (amplified acute responses), 
while Antagonism and Detachment reflect interpersonal dysfunction (impaired 
post-stressor regulation). The fact that different personality dimensions moderate 
different acoustic parameters and stress phases suggests **multivariate specificity** 
rather than global dysregulation.

### Ecological Momentary Assessment: Efficiency Without Compromise

A critical methodological contribution is demonstrating that **15-item EMA** 
(~30 administrations over 2.5 months) achieved **equivalent predictive accuracy** 
to the full 220-item PID-5 baseline assessment (ΔR² < 0.025 across outcomes). 
This is not a limitation but a **validation**: aggregated real-world measurements 
capture the same trait variance as comprehensive questionnaires.

This equivalence offers three advantages:

1. **Reduced recall bias**: EMA assesses personality in-the-moment rather than 
   retrospectively, reducing memory distortion and impression management.

2. **Ecological validity**: Trait expression measured in natural contexts where 
   participants actually experience stress, not artificial laboratory settings.

3. **Efficiency**: 93% reduction in item burden enables repeated measurement 
   without participant fatigue, critical for intensive longitudinal designs.

The ability of brief EMA to match traditional assessment validates its use in 
digital phenotyping contexts where comprehensive questionnaires are impractical. 
For voice-based mental health monitoring, integrating brief EMA personality 
assessment could enable **personalized stress detection** accounting for individual 
differences in baseline reactivity.

### Within-Person Variability: A Complementary Signal

Beyond mean trait levels, we examined whether **emotional lability** (within-person 
variability in EMA scores) uniquely predicted stress responses. Variability showed 
selective effects (5 credible vs 7 for means), specifically moderating **articulatory 
instability** (F2 SD) rather than prosodic parameters (F0).

This dissociation is clinically informative: individuals with high emotional 
dysregulation showed stress-related breakdown in **motor control** (fine articulatory 
adjustments) while preserving affective communication (pitch). Labile individuals 
may maintain social-emotional signaling (prosody) while experiencing subtle 
psychomotor disruption under stress.

This pattern aligns with borderline personality models emphasizing regulatory 
**instability** distinct from affective **intensity** (Linehan, 1993; Crowell et al., 
2009). The fact that lability predicts different acoustic outcomes than mean levels 
demonstrates that intensive designs capture **trait dynamics**—how much people 
fluctuate—missed by single-session assessments.

However, mean trait levels remained the stronger predictor overall, suggesting 
variability provides **complementary** rather than primary information. Future 
research should examine whether variability becomes more predictive in clinical 
samples with greater dysregulation.

### Implications for Voice-Based Digital Phenotyping

Our findings support voice analysis as a viable modality for **unobtrusive stress 
assessment** in naturalistic contexts. Unlike self-report (requires introspection) 
or wearable sensors (require compliance), voice can be collected passively during 
everyday communication.

However, the **modest effect sizes** (stress main effects d = 0.15-0.25; moderation 
effects comparable) underscore that voice alone likely cannot achieve clinical-grade 
stress detection. Personality moderation effects demonstrate that **individual 
differences matter**: a 4 Hz pitch increase might signal high stress for one person 
but low stress for another.

This motivates **personalized models** integrating:
- **Baseline calibration**: Person-specific acoustic norms
- **Personality profiles**: Trait moderation of stress signatures  
- **Context awareness**: Stressor type, social setting, time of day
- **Multimodal fusion**: Voice + linguistic + behavioral data

Digital phenotyping applications should prioritize within-person change detection 
over between-person comparisons, using intensive baseline sampling to establish 
individual acoustic profiles before detecting deviations signaling stress or 
psychopathology.
...
1. **Sample characteristics**: University students facing academic exams represent 
   a specific stress context with high motivation and cognitive engagement. 
   Generalization to clinical populations (where personality pathology is more 
   severe) or other stressors (interpersonal conflict, trauma) requires replication.

2. **Temporal resolution**: Three timepoints (baseline, pre-, post-exam) capture 
   broad stress dynamics but miss finer-grained fluctuations. Future work should 
   examine higher-frequency sampling (multiple times per day) to characterize 
   moment-to-moment variability.

3. **Acoustic feature breadth**: We focused on fundamental frequency and formants 
   as theory-driven markers of arousal and articulation. However, stress may 
   manifest in spectral quality (e.g., breathiness, harshness), temporal patterning 
   (speech rate, pausing), or prosodic contours (intonation trajectories) not 
   examined here. Comprehensive acoustic profiling could reveal additional 
   personality × stress interactions.

4. **Mechanism ambiguity**: Acoustic changes could reflect:
   - **Physiological arousal**: Sympathetic activation → vocal fold tension
   - **Emotional expression**: Communicating distress to others
   - **Cognitive load**: Reduced prosodic expressiveness under demand
   - **Regulatory attempts**: Suppressing or amplifying vocal affect

   Disentangling these pathways requires integrating physiological (cortisol, 
   heart rate) and self-report (subjective stress) data to determine whether 
   voice reflects automatic arousal or strategic communication.

5. **Predictive validity**: We demonstrate personality moderates stress-related 
   acoustic change, but do these patterns **predict clinical outcomes**? Longitudinal 
   research should examine whether individual differences in vocal stress reactivity 
   forecast psychopathology onset, treatment response, or functional impairment.

6. **Generative mechanisms**: Why does Negative Affectivity amplify pitch reactivity 
   while Antagonism affects recovery? Theoretical models must explain domain-specific 
   patterns, potentially linking to biological (HPA axis sensitivity), cognitive 
   (threat appraisal), or social (interpersonal stress generation) mechanisms.

Future research priorities include:

- **Clinical validation**: Replicate in samples with diagnosed personality disorders
- **Intervention studies**: Do treatments reducing personality pathology normalize 
  vocal stress responses?
- **Real-world deployment**: Test passive voice monitoring in naturalistic settings
- **Cross-cultural generalization**: Examine acoustic stress markers in non-Western 
  samples with different display rules

### Conclusion

This study demonstrates that personality pathology dimensions, measured ecologically 
via brief EMA, systematically moderate how academic stress manifests in vocal 
production. Against modest main effects (d = 0.15-0.25), personality interactions 
matched or exceeded these magnitudes, underscoring substantial individual variation 
in stress reactivity.

The equivalence of 15-item EMA to comprehensive 220-item questionnaires validates 
intensive longitudinal assessment for personality research, offering ecological 
validity without sacrificing measurement quality. The complementary role of 
emotional lability—capturing articulatory instability distinct from prosodic 
changes—illustrates the unique information intensive designs provide about 
**trait dynamics**, not just trait levels.

As voice-based digital phenotyping advances, integrating brief personality 
assessment into acoustic stress models will be essential for developing 
**personalized, context-sensitive mental health monitoring**. Our findings 
establish that personality moderates vocal stress signatures, providing a 
foundation for individual difference frameworks in computational paralinguistics 
and affective computing.

The modest effect sizes we observe—both for stress main effects and personality 
moderation—underscore the complexity of voice as a stress biomarker. Clinically 
useful systems will require multimodal integration, within-person calibration, 
and explicit modeling of individual differences. This study provides an empirical 
foundation and methodological template for such work, demonstrating that 
personality traits measured in daily life shape how stress becomes acoustically 
