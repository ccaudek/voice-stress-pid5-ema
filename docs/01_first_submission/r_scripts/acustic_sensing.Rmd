...
abstract: |
  This study examined whether vocal acoustics can capture context-dependent expression of personality pathology during naturalistic stress. Female university students (N = 119) provided voice recordings at three within-subject assessments—baseline, immediately before, and the day after a course examination—while personality pathology traits (PID-5) were assessed via twice-weekly ecological momentary assessment (EMA) over 2.5 months. Exam stress produced dissociable vocal changes: fundamental frequency (F0) increased by 3.27 Hz, indicating heightened autonomic arousal, while normalized noise energy (NNE) decreased by 0.79 dB, suggesting compensatory phonatory control. Personality traits showed domain- and parameter-specific moderation: Negative Affectivity amplified stress-induced F0 increases during anticipatory stress, Antagonism was associated with continued F0 elevation during recovery, suggesting heterogeneity in post-stressor arousal trajectories, and Psychoticism uniquely modulated voice quality (NNE) during the post-stressor period. Detachment and Disinhibition showed minimal moderation, consistent with theoretical predictions distinguishing arousal sensitivity from behavioral control. Methodologically, brief EMA-based personality assessment matched comprehensive baseline questionnaires in predictive accuracy while yielding more precise moderation estimates. These findings support transactional models of personality pathology and indicate that multimodal ambulatory assessment, combining passive acoustic sensing with intensive self-report, can capture how maladaptive traits shape real-world stress responses.
...
Acoustic features were extracted using two complementary approaches targeting distinct aspects of vocal production: (a) traditional phonatory parameters derived from sustained vowel phonations, and (b) mel-frequency cepstral coefficients (MFCCs) extracted from continuous speech.

**Sustained vowel analysis.** Frequency- and time-domain acoustic parameters were extracted from sustained vowel productions using the open-source BioVoice software (Morelli &amp; Manfredi, 2019). Analyses focused on the steady-state portion of each vowel, excluding onset and offset segments to ensure stable phonatory conditions. Measures were computed separately for each vowel token (/a/, /i/, /u/; three repetitions) and then averaged across vowels and repetitions within each recording session, yielding a single session-level estimate per participant.

**Fundamental frequency (F0).** Mean and median F0 were computed as indices of average vocal fold vibratory rate, while the standard deviation of F0 indexed short-term pitch variability. In addition, the temporal location of the maximum F0 value (T0) was extracted as a marker of phonatory dynamics. Fundamental frequency is the most extensively validated acoustic correlate of psychological stress and arousal, reflecting autonomic modulation of laryngeal muscle tension. Experimental and meta-analytic evidence consistently demonstrates reliable increases in F0 under a wide range of stressors, including cognitive load, evaluative threat, and public speaking (Giddens
et al., 2013; Scherer, 2003). Physiologically, these effects are attributed to sympathetic activation of the cricothyroid and vocalis muscles, which increases vocal fold stiffness and elevates vibratory frequency (Titze, 1994).

**Normalized Noise Energy (NNE).** Normalized noise energy (expressed in decibels) quantifies the proportion of aperiodic energy in the voice signal and serves as an index of phonatory quality and glottal closure completeness (Kasuya et al., 1986). Higher NNE values indicate greater noise relative to harmonic energy and are typically associated with incomplete glottal closure and breathier voice quality. Unlike perturbation measures such as jitter and shimmer, NNE captures broader stress-related changes in phonatory control and vocal effort. Empirical findings suggest that acute stress can exert bidirectional effects on vocal noise, increasing NNE through airflow turbulence or decreasing it via compensatory hyperadduction and pressed phonation aimed at maintaining vocal control (Mendoza &amp; Carballo, 1998; Scherer, 2003). Accordingly, NNE may
reflect individual differences in stress-related vocal regulation.

**Selection of acoustic parameters.** Analyses focused on F0 and NNE due to their strong theoretical relevance to stress-related vocal changes and their complementary sensitivity to distinct physiological mechanisms [REF]. Specifically, F0 primarily indexes autonomic arousal, whereas NNE captures stress-related adjustments in phonatory control. This dual-parameter approach aligns with multidimensional models of vocal stress responses (Giddens et al., 2013) and allows testing whether personality traits selectively moderate arousal-related versus control-related components of the vocal stress response. Other acoustic features (e.g., formant frequencies, spectral tilt, speech rate) were not examined, as they are more strongly influenced by linguistic and articulatory factors than by the phonatory physiology most directly affected by autonomic stress responses.
...
All analysis code and de-identified data will be made publicly available upon publication at [OSF LINK]. 
...
These results support the feasibility of combining low-burden repeated trait assessment with objective acoustic markers in longitudinal designs, particularly when comprehensive questionnaires are impractical [Wright et al., 2025]. At the same time, the present design relied on brief standardized voice recordings rather than continuous passive sensing. Extending this approach to more frequent recordings would allow stronger tests of within-person coupling and more detailed modeling of recovery dynamics.
