...
// Joint measurement-and-outcome model for mean Fundamental Frequency (F0)
// × PID-5 personality pathology domains.
//
// Outcome y = session-level F0 (Hz), averaged across sustained vowels /a/, /i/, /u/.
// Positive moderation coefficients (g1, g2) indicate stronger F0 elevation.
...
data {
  ...
  matrix[N_ema, D] X;          // standardised EMA domain scores
}
...
parameters {
  ...
  // Random effects (non-centred): intercept + slopes
...
model {
  ...
  alpha ~ normal(220, 30);   // centred on typical female F0
  b1    ~ normal(0, 10);
  b2    ~ normal(0, 10);
