...
stan_file <- here("stan", "stress", "mfcc_main_effects.stan")
stopifnot(file.exists(stan_file))

dir.create(here("results", "stress", "models"),  recursive = TRUE, showWarnings = FALSE)
dir.create(here("results", "stress", "figures"), recursive = TRUE, showWarnings = FALSE)
...
baseline <- read_excel(mfcc_path, sheet = "Baseline") %>% mutate(timepoint = "baseline")
...
# master_ids <- readRDS(here("results","stress","analysis_bundle.rds"))$id_map$ID
...
mfcc_center <- df_mfcc |> summarise(across(all_of(mfcc_cols), \(x) mean(x))) |> unlist()
mfcc_scale  <- df_mfcc |> summarise(across(all_of(mfcc_cols), \(x) sd(x)))   |> unlist()
...
cat("MFCC DATA: N obs =", nrow(df_stan),
    "| N subj =", N_subj,
    "| K =", K, "\n")
...
desc_stats <- df_mfcc |>
  group_by(timepoint) |>
  summarise(across(all_of(mfcc_cols), \(x) mean(x)), n_obs = n(), .groups = "drop")
...
fit_full$save_object(here("results", "stress", "models", "fit_mfcc_main_effects.rds"))
...
print(loo_cmp)   # full in cima e |elpd_diff| >> se_diff => effetti di condizione utili
...
omnibus <- tibble(
  ...
  D_lo     = c(quantile(post$D_stress, .025),  quantile(post$D_recovery, .025)),
  D_hi     = c(quantile(post$D_stress, .975),  quantile(post$D_recovery, .975))
)
print(omnibus)
...
eff_stress   <- summarise_effect(post, "beta1", mfcc_labels) |> mutate(contrast = "stress")
eff_recovery <- summarise_effect(post, "beta2", mfcc_labels) |> mutate(contrast = "recovery")
...
# Salva tabelle
write_csv(omnibus, here("results", "stress", "tables", "mfcc_omnibus_mahalanobis.csv"))
write_csv(eff_all, here("results", "stress", "tables", "mfcc_per_coefficient_effects.csv"))
...
p_omni <- fit_full$draws(c("D_stress", "D_recovery")) |>
  mcmc_areas(prob = 0.95, prob_outer = 0.99) +
  labs(title = "MFCC: omnibus multivariate effect size",
       subtitle = "Mahalanobis length of the stress / recovery shift vectors") +
  theme_minimal()
ggsave(here("results", "stress", "figures", "mfcc_omnibus_D.png"),
       p_omni, width = 8, height = 4, dpi = 300)

# 11b) Per-coefficiente: intervalli di beta1 (stress) e beta2 (recovery)
p_b1 <- fit_full$draws(paste0("beta1[", 1:K, "]")) |>
  mcmc_intervals(prob = 0.5, prob_outer = 0.95) +
...
p_b2 <- fit_full$draws(paste0("beta2[", 1:K, "]")) |>
  ...
  theme_minimal()
ggsave(here("results", "stress", "figures", "mfcc_beta1_intervals.png"),
       p_b1, width = 7, height = 5, dpi = 300)
ggsave(here("results", "stress", "figures", "mfcc_beta2_intervals.png"),
       p_b2, width = 7, height = 5, dpi = 300)
...
marg <- purrr::map_dfr(1:K, function(k) {
  ...
  tibble(
    ...
    lower  = c(quantile(m_base, .025), quantile(m_pre, .025), quantile(m_post, .025)),
    upper  = c(quantile(m_base, .975), quantile(m_pre, .975), quantile(m_post, .975))
  )
})
p_marg <- ggplot(marg, aes(timepoint, median, group = 1)) +
  ...
  labs(title = "MFCC: model-implied marginal means by timepoint", y = "MFCC (raw units)", x = NULL) +
  theme_minimal()
ggsave(here("results", "stress", "figures", "mfcc_marginal_means.png"),
       p_marg, width = 11, height = 8, dpi = 300)
...
p_ppc <- ggplot() +
  geom_density(data = yrep_long, aes(value, group = .draw),
               colour = "lightblue", linewidth = .2, alpha = .4) +
  geom_density(data = obs_long, aes(value), colour = "navy", linewidth = .8) +
  facet_wrap(~coef, scales = "free") +
  labs(title = "MFCC: posterior predictive check (per coefficient, standardized)") +
  theme_minimal()
ggsave(here("results", "stress", "figures", "mfcc_ppc.png"),
       p_ppc, width = 11, height = 8, dpi = 300)
...
write_csv(as.data.frame(summary_full), here("results", "stress", "tables", "mfcc_posterior_summary.csv"))
write_csv(desc_stats, here("results", "stress", "tables", "mfcc_descriptives.csv"))
...
saveRDS(
  ...
  file = here("results", "stress", "mfcc_analysis_bundle.rds")
)

cat("\n=== ANALYSIS COMPLETE ===\n")
cat("Saved:\n")
cat(" - results/stress/models/fit_mfcc_main_effects.rds\n")
cat(" - results/stress/tables/*.csv  (omnibus, per-coefficient, summary, descriptives)\n")
cat(" - results/stress/figures/*.png (omnibus D, beta intervals, marginal means, PPC)\n")
cat(" - results/stress/mfcc_analysis_bundle.rds\n")
