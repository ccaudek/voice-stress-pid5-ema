...
# Catch item in numerazione del questionario
careless_qpos <- c(68, 161)

# Flag careless (già coerente con il tuo codice)
careless_flag <- (is.na(att1) | att1 != catch_key[1]) &
  (is.na(att2) | att2 != catch_key[2])

df_pid5$careless <- careless_flag

# Funzione: item di un dominio (senza catch)
domain_items <- function(domain_name) {
  facets <- domain_facets[[domain_name]]
  items <- unlist(facet_items[facets])
  setdiff(items, careless_qpos)
}

pid5_domains_items <- map(
  names(domain_facets),
  domain_items
)
names(pid5_domains_items) <- names(domain_facets)

compute_reliability <- function(data, item_numbers) {
  cols <- paste0("i_", item_numbers)
  mat <- data[, cols]
  ...
  list(
    ...
    omega = psych::omega(mat, plot = FALSE, warnings = FALSE)$omega.tot
  )
}

rel_domains_full <- map_df(
  names(pid5_domains_items),
  function(dom) {
    rel <- compute_reliability(df_pid5, pid5_domains_items[[dom]])
    tibble(
      domain = dom,
      sample = "full",
      alpha = rel$alpha,
...
# Versione CLEAN (careless esclusi)

df_pid5_clean <- df_pid5 %>% filter(!careless)

rel_domains_clean <- map_df(
  names(pid5_domains_items),
  function(dom) {
    rel <- compute_reliability(df_pid5_clean, pid5_domains_items[[dom]])
    tibble(
      domain = dom,
      sample = "clean",
      alpha = rel$alpha,
...
all_items <- setdiff(seq_along(item_idx), careless_qpos)

rel_total_full <- {
  rel <- compute_reliability(df_pid5, all_items)
  tibble(
    domain = "total_PID5",
    sample = "full",
    alpha = rel$alpha,
...
rel_total_clean <- {
  rel <- compute_reliability(df_pid5_clean, all_items)
  tibble(
    domain = "total_PID5",
    sample = "clean",
    alpha = rel$alpha,
...
reliability_results
# A tibble: 12 × 4
# domain          sample alpha omega
# 1 negative_affect full   0.783 0.851
# 2 detachment      full   0.754 0.844
# 3 antagonism      full   0.670 0.819
# 4 disinhibition   full   0.747 0.843
# 5 psychoticism    full   0.870 0.898

# 6 negative_affect clean  0.767 0.843
# 7 detachment      clean  0.741 0.840
# 8 antagonism      clean  0.627 0.796
# 9 disinhibition   clean  0.725 0.824
# 10 psychoticism    clean  0.864 0.892

# 11 total_PID5      full   0.965 0.979
# 12 total_PID5      clean  0.964 0.977
